...
class BulletClient(object):
    ...
    """
        ...
        self._client = pybullet.connect(connection_mode,
                                        options='--background_color_red=1.0 --background_color_green=1.0 --background_color_blue=1.0 --width=960 --height=540')
...
class Scene:
    ...
    def __init__(self, bullet_client, gravity, timestep, frame_skip, scene):
        self._p = bullet_client
...
class StadiumScene(Scene):
    ...
    def initialize(self):
        ...
        if os.path.exists(self.scene):
            ...
            self._planeId = self._p.loadURDF(self.scene, [0, 0, 0], z2y, useMaximalCoordinates=True, globalScaling=10.)
        else:
            z2y = self._p.getQuaternionFromEuler([-math.pi * 0.5, 0, 0]) 
            self._planeId = self._p.loadURDF(R"plane_implicit.urdf", [0, 0, 0], z2y, useMaximalCoordinates=True)
            
        self._p.configureDebugVisualizer(self._p.COV_ENABLE_Y_AXIS_UP, 1)
        self._p.changeDynamics(self._planeId, linkIndex=-1, lateralFriction=0.9)

        self.ground_plane_mjcf = (self._planeId,)
...
class Camera:
    def __init__(self, bc, fps=60, dist=2.5, yaw=0, pitch=0):
        self._p = bc
        self._cam_dist = dist
        self._cam_yaw = yaw
    ...
    def dump_rgb_array(self):
        ...
        # self._p.configureDebugVisualizer(lightPosition=np.array([5, 5, 100]))
