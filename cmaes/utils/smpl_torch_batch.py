...
class SMPLModel(Module):
    def __init__(self, device=torch.device('cpu'), model_path='./SMPL_NEUTRAL.pkl',
        ...
        self.joint_regressor = torch.from_numpy(
            np.load('data/J_regressor_lsp.npy')
        ).type(self.data_type)
        self.halpe_regressor = torch.from_numpy(
        ...
        self.shapedirs = torch.from_numpy(np.array(params['shapedirs'])).type(self.data_type)
        ...
        for name in ['J_regressor', 'joint_regressor', 'halpe_regressor', 'weights', 'posedirs', 'v_template', 'shapedirs']:
    ...
    def forward(self, betas, thetas, trans, gR=None, lsp=True):
        ...
        #self.pose = torch.nn.Parameter(thetas, requires_grad=True)
        #self.shape = torch.nn.Parameter(betas, requires_grad=True)
...
if __name__ == '__main__':
    ...
    dataset_dir = 'F:\BuzhenHuang_Data\VCLO50K dataset\VCLO50K images'
    human_mask_dir = 'F:\BuzhenHuang_Data\VCLO50K dataset\VCLO50K masks'
    generator = UV_Map_Generator(
        UV_height=256,
        UV_pickle='./data_utils/radvani_template.pickle'
    )
    model = SMPLModel(
        device=device,
    ...
    train_dataset = GetData_vclo(train=True, num_train=51000, virtual_mask=True, dataset_dir=dataset_dir,
                            mask_dir=human_mask_dir, model=model, generator=generator)
    shape= train_dataset.shape
    pose = train_dataset.pose
    shape = torch.from_numpy(shape).type(torch.FloatTensor).resize(1, 10).to(device)
    pose = torch.from_numpy(pose).type(torch.FloatTensor).resize(1, 72).to(device)
    _trans = torch.zeros((1, 3),dtype=torch.float32, device=device)
    mesh, lsp_joints = model(shape, pose, _trans)
    model.write_obj(mesh[0], 'paper.obj')
    print('ok')