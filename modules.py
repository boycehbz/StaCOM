...
from utils.FileLoaders import save_pkl, load_obj, write_obj
import sys
...
def init(note='occlusion', dtype=torch.float32, mode='eval', model_type='smpl', output='output', **kwargs):
    ...
    if mode == 'eval':
        ...
        logger.set_names(['Epoch', 'LR', 'Train Loss', 'C_Precision', 'C_Recall', 'C_F1', 'IDF', 'Pen'])
...
class LossLoader():
    def __init__(self, smpl, train_loss='L1', test_loss='L1', device=torch.device('cpu'), **kwargs):
        ...
        self.device = device
        self.smpl = smpl
        self.diversity_samples = []  
        self.contact_predictions = []
        self.contact_ground_truth = []
        self.idf_loss = []
        self.pen_loss = []
        self.single_amp_trainer = None
        self.interact_amp_trainer = None

        if 'Single_AMP_Loss' in self.train_loss_type:
            from model.model.amp_discriminator import AMPTrainer
            amp_cmu_dir = kwargs.get('amp_cmu_dir', '')
            amp_use_pretrained = kwargs.get('amp_use_pretrained', False)
            amp_ckpt_path = kwargs.get('amp_discriminator_ckpt', '')
            self.single_amp_trainer = AMPTrainer(device=device, cmu_motion_dir=amp_cmu_dir)
            print("Single-person AMP discriminator initialized")
            if self.single_amp_trainer is not None and amp_use_pretrained and amp_ckpt_path:
                self.single_amp_trainer.load_discriminator(amp_ckpt_path)
                print("load single disc successful")
        if 'Interact_AMP_Loss' in self.train_loss_type:
            from model.amp_discriminator import InteractionAMPTrainer
            self.interact_amp_trainer = InteractionAMPTrainer(device=device)
            print("Interaction AMP discriminator initialized")
            interact_use_pretrained = kwargs.get('interact_amp_use_pretrained', amp_use_pretrained)
            interact_ckpt = kwargs.get('interact_amp_discriminator_ckpt', '')
            if self.interact_amp_trainer is not None and interact_use_pretrained and interact_ckpt:
                self.interact_amp_trainer.load_discriminator(interact_ckpt)
                print("load interact disc successful")
        ...
        for loss in self.train_loss_type:
            ...
            if loss == 'Diffusion_Loss':
                self.train_loss.update(Diffusion_Loss=Diffusion_Loss(self.device))
            if loss == 'Flow_Loss':
                self.train_loss.update(Flow_Loss=Flow_Loss(self.device))
            if loss == 'Single_AMP_Loss':
                single_weight = kwargs.get('single_amp_weight', 0.5)
                self.train_loss.update(Single_AMP_Loss=Single_AMP_Loss(weight=single_weight))
            if loss == 'Interact_AMP_Loss':
                interact_weight = kwargs.get('interact_amp_weight', 0.5)
                self.train_loss.update(Interact_AMP_Loss=Interact_AMP_Loss(weight=interact_weight))
        ...
        for loss in self.test_loss_type:
            ...
            if loss == 'Diversity':
                self.test_loss.update(Diversity=Diversity(self.device))
            if loss == 'ContactDetection':
                self.test_loss.update(ContactDetection=ContactDetection(self.device))
            if loss == 'IDFAccuracy':
                self.test_loss.update(IDFAccuracy=IDFAccuracy(self.device, 
                                                 num_boundary_points=8, 
                                                 num_surface_points=16))
            if loss == 'AffordanceLoss':
                self.test_loss.update(AffordanceLoss=AffordanceLoss(self.device))
            if loss == 'AffordanceConsistencyLoss':
                self.test_loss.update(AffordanceConsistencyLoss=AffordanceConsistencyLoss(self.device))
            if loss == 'Penetration':
                self.test_loss.update(Penetration=Penetration(self.smpl, self.device))
    ...
    def calcul_trainloss(self, pred, gt):
        ...
        for ltype in self.train_loss:
            if ltype == 'L1':
                loss_dict.update(L1=self.train_loss['L1'](pred['pred_x'], gt['x'], gt['valid']))
            elif ltype == 'L2':
                loss_dict.update(L2=self.train_loss['L2'](pred['pred_x'], gt['x'], gt['valid']))
                ...
                Flow_Loss = self.train_loss['Flow_Loss'](pred['pred_u_t'], pred['u_t'])
                loss_dict = {**loss_dict, **Flow_Loss}
            elif ltype == 'Pen_Loss':
            ...
            elif ltype == 'Diffusion_Loss':
                Diffusion_Loss = self.train_loss['Diffusion_Loss'](pred['pred_x0'], pred['pred_noise'], 
                                                                   pred['target'], pred['objective'], pred['loss_weight'])
                loss_dict = {**loss_dict, **Diffusion_Loss}
            # Calculate your loss here
            elif ltype == 'Single_AMP_Loss' and 'Single_AMP_Loss' in self.train_loss and self.single_amp_trainer is not None:
                pred_pose6d = pred['pred_pose6d']
                pred_betas = pred['pred_shape']
                pred_rotmat = rotation_6d_to_matrix(pred_pose6d).view(-1, 24, 3, 3) 

                real_body_rotmat = None
                real_betas = None

                amp_loss = self.train_loss['Single_AMP_Loss'](
                    pred_rotmat,
                    pred_betas,
                    self.single_amp_trainer,
                    real_body_rotmat=real_body_rotmat,
                    real_betas=real_betas
                )
                loss_dict = {**loss_dict, **amp_loss}
            elif ltype == 'Interact_AMP_Loss' and 'Interact_AMP_Loss' in self.train_loss and self.interact_amp_trainer is not None:
                interact_loss = self.train_loss['Interact_AMP_Loss'](pred, gt, self.interact_amp_trainer)
                loss_dict = {**loss_dict, **interact_loss}
            else:
                pass
        loss = 0
        for k in loss_dict:
    ...
    def calcul_testloss(self, pred, gt):
        ...
        for ltype in self.test_loss:
            if ltype == 'L1':
                loss_dict.update(L1=self.test_loss['L1'](pred['pred_x'], gt['x'], gt['valid']))
            elif ltype == 'MPJPE':
            ...
            elif ltype == 'Diversity':
                if 'pred_joints' in pred:
                    self.diversity_samples.append(pred['pred_joints'].detach().cpu())
                    loss_dict.update(Diversity=torch.tensor(0.0, device=self.device))
            elif ltype == 'ContactDetection':
                contact_result = self.test_loss['ContactDetection'](
                    pred['pred_joints'], gt['gt_joints'], gt['obj_points'], gt['obj_pose'])
                self.contact_predictions.append(contact_result['pred_contacts'].detach().cpu())
                self.contact_ground_truth.append(contact_result['gt_contacts'].detach().cpu())
                loss_dict.update(contact_precision=torch.tensor(0.0, device=self.device))
                loss_dict.update(contact_recall=torch.tensor(0.0, device=self.device))
                loss_dict.update(contact_f1=torch.tensor(0.0, device=self.device))
            elif ltype == 'IDFAccuracy':
                idf_loss = self.test_loss['IDFAccuracy'](
                    pred['pred_joints'], gt['gt_joints'], gt['obj_points'], gt['obj_pose'])
                loss_dict.update(IDFAccuracy=idf_loss)
                self.idf_loss.append(idf_loss.detach().cpu().numpy())
            elif ltype == 'Penetration':
                pen_loss = self.test_loss['Penetration'](
                    pred['pred_verts'], gt['obj_points'], gt['data_shape'], gt['valid'])
                loss_dict.update(Penetration=pen_loss)
                self.pen_loss.append(idf_loss.detach().cpu().numpy())
            else:
        ...
        for k in loss_dict:
            ...
            if isinstance(loss_dict[k], dict):
                for sub_k, sub_v in loss_dict[k].items():
                    loss_dict[sub_k] = round(float(sub_v.detach().cpu().numpy()), 3)
                del loss_dict[k]
            else:
                loss += loss_dict[k]
    ...
    def compute_final_metrics(self):
        final_metrics = {}
        if len(self.diversity_samples) > 1:
            all_samples = torch.cat(self.diversity_samples, dim=0)
            if len(all_samples) >= 2:
                diversity_scores = []
                for i in range(len(all_samples)):
                    for j in range(i + 1, len(all_samples)):
                        dist = torch.mean((all_samples[i] - all_samples[j]) ** 2)
                        diversity_scores.append(dist)
                if diversity_scores:
                    diversity = torch.stack(diversity_scores).mean()
                    final_metrics['Diversity'] = float(diversity.detach().cpu().numpy())
                else:
                    print("metric error")
                    final_metrics['Diversity'] = 0.0
            else:
                print("metric error")
                final_metrics['Diversity'] = 0.0
        else:
        ...
        if len(self.contact_predictions) > 0 and len(self.contact_ground_truth) > 0:
            all_pred_contacts = torch.cat(self.contact_predictions, dim=0)  # [B, T, joints]
            all_gt_contacts = torch.cat(self.contact_ground_truth, dim=0)    # [B, T, joints]
            
            pred_contact_frames = (all_pred_contacts.sum(dim=-1) > 0).float()  
            gt_contact_frames = (all_gt_contacts.sum(dim=-1) > 0).float()            
            pred_flat = pred_contact_frames.view(-1)
            gt_flat = gt_contact_frames.view(-1)
            
            tp = ((pred_flat == 1) & (gt_flat == 1)).sum().float()
            fp = ((pred_flat == 1) & (gt_flat == 0)).sum().float()
            fn = ((pred_flat == 0) & (gt_flat == 1)).sum().float()
            
            print(f"Frame-level TP: {tp}, FP: {fp}, FN: {fn}")
            
            precision = tp / (tp + fp + 1e-8)
            recall = tp / (tp + fn + 1e-8)
            f1_score = 2 * precision * recall / (precision + recall + 1e-8)
            
            final_metrics['contact_precision'] = float(precision.detach().cpu().numpy())
            final_metrics['contact_recall'] = float(recall.detach().cpu().numpy()) 
            final_metrics['contact_f1'] = float(f1_score.detach().cpu().numpy())
        else:
            print("contact metric error")
            final_metrics['contact_precision'] = 0.0
            final_metrics['contact_recall'] = 0.0
            final_metrics['contact_f1'] = 0.0
        
        if len(self.idf_loss) > 0:
            final_metrics['IDFAccuracy'] = np.array(self.idf_loss).mean()
        else:
            final_metrics['IDFAccuracy'] = 0.0

        if len(self.pen_loss) > 0:
            final_metrics['Penetration'] = np.array(self.pen_loss).mean()
        else:
            final_metrics['Penetration'] = 0.0
...
class ModelLoader():
    def __init__(self, dtype=torch.float32, out_dir='', device=torch.device('cpu'), model=None, lr=0.001, pretrain=False, pretrain_dir='', batchsize=32, task=None, data_folder='', use_prior=False, testset='', test_loss='MPJPE', frame_length=16, model_type='smpl', **kwargs):
        ...
        self.model = eval(self.model_type)(self.model_smpl_gpu, num_joints=num_joint, frame_length=self.frame_length)
    ...
    def save_model(self, epoch, task):
        # save trained model
        output = os.path.join(self.output, 'trained_model')
    ...
    def save_generated_interaction(self, results, iter, batchsize):
        ...
        for batch, (pred_trans, pred_pose, pred_shape, pred_verts, gt_trans, gt_pose, gt_shape, gt_verts) in enumerate(zip(results['pred_trans'], results['pred_pose'], results['pred_shape'], results['pred_verts'], results['gt_trans'], results['gt_pose'], results['gt_shape'], results['gt_verts'])):
            ...
            for f, (p_trans, p_pose, p_shape, p_verts, g_trans, g_pose, g_shape, g_verts) in enumerate(zip(pred_trans, pred_pose, pred_shape, pred_verts, gt_trans, gt_pose, gt_shape, gt_verts)):
                ...
                centroid = p_verts.mean(axis=(0, 1))
                centroid[:2] = 0
                p_verts = np.matmul((p_verts - centroid), aroundx) + centroid

                centroid = g_verts.mean(axis=(0, 1))
                centroid[:2] = 0
                g_verts = np.matmul((g_verts - centroid), aroundx) + centroid

                pred_smpl_front = renderer.render_front_view(p_verts)
                pred_smpl_side = renderer.render_side_view(p_verts)
                pred_smpl_backside = renderer.render_backside_view(p_verts)
                pred = np.concatenate((pred_smpl_front, pred_smpl_side, pred_smpl_backside), axis=1)

                gt_smpl_front = renderer.render_front_view(g_verts)
                gt_smpl_side = renderer.render_side_view(g_verts)
                gt_smpl_backside = renderer.render_backside_view(g_verts)
                gt = np.concatenate((gt_smpl_front, gt_smpl_side, gt_smpl_backside), axis=1)
    ...
    def save_results(self, results, iter, batchsize):
            ...
            batch_size, frame_length, agent_num = results['data_shape']
            ...
            results['pred_verts'] = results['pred_verts'].reshape(batch_size, frame_length, 2, -1, 3)
            results['gt_verts'] = results['gt_verts'].reshape(batch_size, frame_length, 2, -1, 3)
            ...
            try:
                for index, (seq_pred_verts, seq_gt_verts, seq_img, obj_path, seq_obj_pose) in enumerate(zip(results['pred_verts'], results['gt_verts'], results['imgs'], results['obj_path'], results['obj_pose'])):
                    if index > 0:
                        break
                    
                    obj_template, obj_faces = load_obj(obj_path)
                    obj_template = np.insert(obj_template, 3, 1, axis=1)

                    for frame_idx, (pred_verts, gt_verts, img, obj_pose) in enumerate(zip(seq_pred_verts, seq_gt_verts, seq_img, seq_obj_pose)):

                        obj_pose = obj_pose.reshape(4, 4)
                        obj_verts = (np.dot(obj_pose, obj_template.T).T)[:,:3]

                        name = img.replace(self.data_folder + '/', '').replace('\\', '_').replace('/', '_')[:-4]
                        ...
                        if renderer is None:
                            renderer = Renderer_HOI(focal_length=focal_length, center=[cx, cy], img_w=2048, img_h=2048)

                        pred_verts = ((obj_verts, obj_faces), (pred_verts[0], self.model_smpl_gpu.faces), (pred_verts[1], self.model_smpl_gpu.faces), )
                        gt_verts = ((obj_verts, obj_faces), (gt_verts[0], self.model_smpl_gpu.faces), (gt_verts[1], self.model_smpl_gpu.faces), )

                        pred_smpl = renderer.render_front_view(pred_verts)
                        pred_smpl_side = renderer.render_side_view(pred_verts)
                        pred_smpl = np.concatenate((pred_smpl, pred_smpl_side), axis=1)

                        gt_smpl = renderer.render_front_view(gt_verts)
                        gt_smpl_side = renderer.render_side_view(gt_verts)
                        gt_smpl = np.concatenate((gt_smpl, gt_smpl_side), axis=1)
    ...
    #     # results['valid'] = results['valid'].reshape(-1, 2)
    #     results['pred_verts'] = results['pred_verts'].reshape(batch_size, frame_length, 2, -1, 3)
    #     results['gt_verts'] = results['gt_verts'].reshape(batch_size, frame_length, 2, -1, 3)
    ...
    #         for index, (pred_verts, gt_verts, img, obj_pose) in enumerate(zip(seq_pred_verts, seq_gt_verts, seq_img, seq_obj_pose)):
    ...
    #             pred_verts = ((pred_verts[0], self.model_smpl_gpu.faces), (pred_verts[1], self.model_smpl_gpu.faces), (obj_verts, obj_faces))
    #             gt_verts = ((gt_verts[0], self.model_smpl_gpu.faces), (gt_verts[1], self.model_smpl_gpu.faces), (obj_verts, obj_faces))
    ...
    #             #     background = cv2.putText(background, 'valid 00: ' + str(results['valid'][index][0]), (50,150),cv2.FONT_HERSHEY_COMPLEX,2,(105,170,255),5)
    #             #     background = cv2.putText(background, 'valid 01: ' + str(results['valid'][index][1]), (50,350),cv2.FONT_HERSHEY_COMPLEX,2,(255,191,105),5)
    ...
    #             #     mesh_name = os.path.join(self.output, 'meshes/%s_%02d_pred_%02d_mesh.obj' %(name, iter * batchsize + index, hidx))
    #             #     write_obj(pred_vert[0], pred_vert[1], mesh_name)

    #             #     mesh_name = os.path.join(self.output, 'meshes/%s_%02d_gt_%02d_mesh.obj' %(name, iter * batchsize + index, hidx))
    #             #     write_obj(gt_vert[0], gt_vert[1], mesh_name)
    #             renderer.delete()
    #             del renderer
    #             import gc
    #             gc.collect()
    ...
    def save_single_results(self, results, iter, batchsize):
        output = os.path.join(self.output, 'images')
        if not os.path.exists(output):
            os.makedirs(output)
        ...
        results['pred_verts'] = results['pred_verts'].reshape(results['focal_length'].shape[0], 1, -1, 3)
        results['gt_verts'] = results['gt_verts'].reshape(results['focal_length'].shape[0], 1, -1, 3)
        ...
        for index, (img, pred_verts, gt_verts, focal) in enumerate(zip(results['imgs'], results['pred_verts'], results['gt_verts'], results['focal_length'])):
            ...
            background = np.zeros_like(img)
            if 'MPJPE' in results.keys():
                background = cv2.putText(background, 'MPJPE: ' + str(results['MPJPE'][index]), (50,150),cv2.FONT_HERSHEY_COMPLEX,2,(105,170,255),5)
    ...
    def save_init_optimization_results(self, results, iter, batchsize):
        ...
        results['pred_verts'] = results['pred_verts'] + results['pred_trans'][:,np.newaxis,:]
        results['gt_verts'] = results['gt_verts'] + results['gt_trans'][:,np.newaxis,:]
        results['hmr2_verts'] = results['hmr2_verts'] + results['hmr2_trans'][:,np.newaxis,:]
        
        results['pred_verts'] = results['pred_verts'].reshape(results['focal_length'].shape[0], 1, -1, 3)
        results['gt_verts'] = results['gt_verts'].reshape(results['focal_length'].shape[0], 1, -1, 3)
        results['hmr2_verts'] = results['hmr2_verts'].reshape(results['focal_length'].shape[0], 1, -1, 3)

        for index, (img, pred_verts, gt_verts, hmr2_verts, focal) in enumerate(zip(results['imgs'], results['pred_verts'], results['gt_verts'], results['hmr2_verts'], results['focal_length'])):
            ...
            hmr2_smpl = renderer.render_front_view(hmr2_verts, bg_img_rgb=img.copy())
            hmr2_smpl_side = renderer.render_side_view(hmr2_verts)
            hmr2_smpl = np.concatenate((img, hmr2_smpl, hmr2_smpl_side), axis=1)

            rendered = np.concatenate((pred_smpl, hmr2_smpl, gt_smpl), axis=0)
...
class DatasetLoader():
    ...
    def load_trainset(self):
        ...
        for i in range(len(self.trainset)):
            if self.task == 'hoi':
                train_dataset.append(HOI_Data(True, self.dtype, self.data_folder, self.trainset[i], self.smpl, frame_length=self.frame_length))
    ...
    def load_testset(self):
        ...
        for i in range(len(self.testset)):
            if self.task == 'hoi':
                test_dataset.append(HOI_Data(False, self.dtype, self.data_folder, self.testset[i], self.smpl, frame_length=self.frame_length))
