...
class SMPLXModel(Module):
    def __init__(self, device=torch.device('cpu'), model_path='./SMPL_NEUTRAL.pkl',
        ...
        self.joint_regressor_halpe = torch.from_numpy(np.load('data/J_regressor_halpe_smplx.npy')).to(device)

    def forward(self, betas, thetas, trans, gR=None, halpe=False, joints_only=False):
        betas = betas
        global_orient = thetas[:,:3]
        body_pose = thetas[:,3:66]
        transl = trans
        if joints_only:
            output = self.model(betas, global_orient, body_pose, 
...
class SMPLModel(Module):
    ...
    def forward(self, betas, thetas, trans, gR=None, halpe=False, lsp=False, joints_only=False):
