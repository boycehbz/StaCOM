...
class InterhumanFlow_Matching(nn.Module):
    def __init__(
        self,
        smpl,
        num_joints=21,
        ...
        self.object_condition_dim = self.object_pose_feature_dim
    ...
    def condition_process(self, data):
        ...
        if self.use_object_attention:
            obj_features = self.object_feature_proj(object_pose_feat)
        else:
            obj_features = None
    ...
    def sample_with_condition(
        ...
        """
            带条件的Flow Matching采样
            original flow matching 没有额外的约束
        """
...
class interhuman_flow_matching(InterhumanFlow_Matching):
    """Backwards-compatible alias preserving the original snake_case naming."""


__all__ = [
    'InterhumanFlow_Matching',
    'interhuman_flow_matching',
]
