...
class TransformerBlock(nn.Module):
    def __init__(self,
        ...
        self.object_ca_block = VanillaCrossAttention(latent_dim, latent_dim, num_heads, dropout, latent_dim)
        self.ffn = FFN(latent_dim, ff_size, dropout, latent_dim)

    def forward(self, x, y, emb=None, key_padding_mask=None, obj_features=None):
        h1 = self.sa_block(x, emb, key_padding_mask)
        h1 = h1 + x
        h2 = self.ca_block(h1, y, emb, key_padding_mask)
        h2 = h2 + h1
        if obj_features is not None:
            h3 = self.object_ca_block(h2, obj_features, emb, key_padding_mask)
            h2 = h2 + h3
        out = self.ffn(h2, emb)
        out = out + h2
        return out
