...
from model.affordance import Affordance3DBranch, AffordanceConditioner
from model.pointnet import ContactToAffordanceConverter


class interhuman_diffusion_pointnet(nn.Module):
    def __init__(self, smpl, num_joints=21, latentD=32, frame_length=16, n_layers=1, hidden_size=256, bidirectional=True,):
        super(interhuman_diffusion_pointnet, self).__init__()
        self.smpl = smpl
        ...
        self.pointnet_extractor = PointNet2MSG(
            input_dim=6,  # xyz + normal
            output_dim=self.latent_dim,  # 512
            num_points=100
        )
        self.use_affordance = True
        if self.use_affordance:
            self.affordance_3d_branch = Affordance3DBranch(
                input_dim=6,
                hidden_dim=self.latent_dim,
                num_points=100
            )
            self.affordance_conditioner = AffordanceConditioner(
                affordance_dim=100,
                latent_dim=self.latent_dim
            )
            self.contact_to_affordance = ContactToAffordanceConverter(
                contact_threshold=0.05,
                affordance_radius=0.1
            )
            self.affordance_weight = 0.3
    ...
    def condition_process(self, data):
        ...
        obj_points_flat = obj_points.view(-1, obj_points.shape[-2], 6)
        point_features = self.pointnet_extractor(obj_points_flat)
        point_features = point_features.view(batch_size, frame_length, -1)
        img_info['obj_features'] = point_features

        point_features_projected = self.point_project(point_features)
        affordance_scores = self.affordance_3d_branch(obj_points_flat)
        affordance_scores = affordance_scores.view(batch_size, frame_length, 100)
        temporal_weights = self.compute_temporal_weights(data.get('obj_pose', None))
        affordance_features = self.affordance_conditioner(affordance_scores, temporal_weights)
        img_info['affordance_scores'] = affordance_scores
        img_info['temporal_weights'] = temporal_weights
        cond = traj_cond + point_features_projected + self.affordance_weight * affordance_features
        # cond = traj_cond + point_features_projected
    ...
    def inference(self, x_t, t, cond, img_info, data, mean):
        ...
        x_a, x_b = x_t[:,:,0], x_t[:,:,1]
        t = t[:,None].repeat(1, frame_length)

        mask = None
        if mask is not None:
            mask = mask[...,0]

        emb = self.embed_timestep(t.reshape(-1)).reshape(batch_size, frame_length, -1) + self.feature_embed(cond)
        ...
        obj_local_features = img_info['obj_features']

        for i,block in enumerate(self.blocks):
            h_a = block(h_a_prev, h_b_prev * counterpart_mask, emb, key_padding_mask, obj_features=obj_local_features)
            h_b = block(h_b_prev, h_a_prev * counterpart_mask, emb, key_padding_mask, obj_features=obj_local_features)
            h_a_prev = h_a
            h_b_prev = h_b
