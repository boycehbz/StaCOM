...
class InterhumanFlow_BPS_SingleDisc(nn.Module):
    def __init__(
        self,
        smpl,
        num_joints=21,
    ...
    def condition_process(self, data):
        ...
        if 'contact_points' in data:
            ...
            contact_points = torch.zeros(
                batch_size,
                frame_length,
                self.max_agents,
                self.num_hand_tokens,
                3,
                device=device,
            )

        if 'contact_valid' in data:
            contact_valid = data['contact_valid'].float()
        else:
            contact_valid = torch.zeros(
                batch_size,
                frame_length,
                self.max_agents,
                self.num_hand_tokens,
        ...
        contact_cond_input = torch.cat([contact_flat, contact_valid_flat], dim=-1)
        contact_cond = self.contact_project(contact_cond_input)
        cond_features.append(contact_cond)

        cond = torch.stack(cond_features, dim=0).mean(dim=0)

        if self.use_object_attention:
            contact_normals = data.get('contact_normals') if isinstance(data, dict) else None
            if contact_normals is None:
                contact_normals = torch.zeros_like(contact_points)
...
class interhuman_flow_BPS_disc(InterhumanFlow_BPS_SingleDisc):
    """Backwards-compatible alias preserving the original naming convention."""


__all__ = [
    'InterhumanFlow_BPS_SingleDisc',
    'interhuman_flow_BPS_disc',
]
